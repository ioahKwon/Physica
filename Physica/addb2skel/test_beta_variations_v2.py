...
V2: Fixed coordinate system issue - no conversion needed.
"""
...
def main():
    ...
    # Load AddB joints (NO conversion - compare directly)
    print(f"Loading AddB joints from {B3D_PATH}...")
    addb_joints, joint_names, metadata = load_b3d(B3D_PATH)
    addb_joints = addb_joints[0]  # First frame, no coordinate conversion
    print(f"  AddB joints shape: {addb_joints.shape}")

    # Joint mapping (AddB idx, SKEL idx) - scapula mapping
    joint_mapping = [
        (0, 0),   # pelvis
        (1, 1),   # hip_r → femur_r
