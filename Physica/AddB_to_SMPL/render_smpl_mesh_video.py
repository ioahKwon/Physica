#!/usr/bin/env python3
"""
Render SMPL mesh + skeleton from analytical foot results as video
"""
...
def render_mesh_wireframe(img, verts_2d, faces, depth_order, color=(100, 150, 255), thickness=1):
    ...
    Args:
        ...
        verts_2d: [N, 2] 2D vertex positions
        faces: [M, 3] face indices
        depth_order: [M] depth values for each face (for sorting)
        color: edge color
...
def render_frame(verts_3d, joints_3d, faces, img_width, img_height):
    ...
    Args:
        verts_3d: [N, 3] 3D vertices
        joints_3d: [24, 3] 3D joint positions
        faces: [M, 3] face indices
...
def main():
    smpl_params_path = '/tmp/P020_analytical_foot/foot_orient_loss_with_arm_carter2023_p020/smpl_params.npz'
    smpl_model_path = '/egr/research-zijunlab/kwonjoon/Code/Physica/AddB_to_SMPL/models/smpl_model.pkl'
    output_path = '/egr/research-zijunlab/kwonjoon/smpl_mesh_p020_analytical_foot.mp4'

    print("SMPL Mesh + Skeleton 비디오 생성 중...")
    ...
    for i in range(len(verts)):
        ...
        cv2.putText(img, 'SMPL Analytical Foot (FIX 4)', (10, 70),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 150, 0), 2)
        cv2.putText(img, f'MPJPE: 57.11mm', (10, 110),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 150, 0), 2)
