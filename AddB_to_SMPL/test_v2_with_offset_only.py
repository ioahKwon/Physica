#!/usr/bin/env python3
"""
Test v2 baseline + per-joint offset refinement only

This script uses the EXACT v2 pipeline and only adds per-joint offset refinement
at the end to see the isolated effect.
"""
...
class V2WithOffsetTester:
    ...
    def optimize_shape_v2(self):
        ...
        print("\nStage 1: Shape optimization (v2 baseline)")
    ...
    def optimize_pose_v2(self, betas):
        ...
        print("\nStage 2: Pose optimization (v2 baseline)")
        for t in range(self.n_frames):
            if t % 200 == 0 or t == self.n_frames - 1:
                print(f"  Frame {t:4d}/{self.n_frames}: optimizing...")

            target_frame = self.target_joints[t]
            if torch.isnan(target_frame).all():
                continue

            pose_param = nn.Parameter(poses[t].clone())
            trans_param = nn.Parameter(trans[t].clone())
            optimizer = torch.optim.Adam([pose_param, trans_param], lr=self.pose_lr)

            for _ in range(self.pose_iters):
                optimizer.zero_grad()
                ...
                # Per-joint weighting (v2 baseline: ankles and spine)
                weights = torch.ones(diff.shape[0], device=self.device)
                for i, smpl_idx in enumerate([self.smpl_indices[j] for j, m in enumerate(mask) if m]):
    ...
    def run_tests(self):
        print("=" * 80)
        print("Testing v2 baseline + Per-Joint Offset Only")
        print("=" * 80)

        # Test 1: v2 baseline
        print("\n" + "=" * 80)
        print("TEST 1: v2 BASELINE (no offsets)")
        print("=" * 80)

        betas = self.optimize_shape_v2()
        poses, trans = self.optimize_pose_v2(betas)
        ...
        print(f"\n✓ v2 Baseline MPJPE: {baseline_mpjpe:.2f} mm")

        # Test 2: v2 + per-joint offsets
        print("\n" + "=" * 80)
        print("TEST 2: v2 + PER-JOINT OFFSETS")
        print("=" * 80)
        ...
        print(f"\n✓ v2 + Offsets MPJPE: {offset_mpjpe:.2f} mm")

        # Summary
        print("\n" + "=" * 80)
        print("SUMMARY")
        print("=" * 80)
        print(f"v2 Baseline:          {baseline_mpjpe:7.2f} mm")
        print(f"v2 + Per-Joint Offsets: {offset_mpjpe:7.2f} mm ({offset_mpjpe - baseline_mpjpe:+.2f} mm)")
        print("=" * 80)


def main():
    parser = argparse.ArgumentParser(description="Test v2 baseline + per-joint offset only")
    ...
    # Run tests
    tester = V2WithOffsetTester(args.b3d, smpl, device)
    tester.run_tests()
