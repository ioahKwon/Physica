#!/usr/bin/env python3
"""
Side-by-side visualization for worst case (Tiziana2019 Subject12)
Left: B3D ground truth joints
Right: SMPL predicted joints
"""
...
def main():
    ...
    print("\n" + "="*80)
    print("SIDE-BY-SIDE WORST CASE VISUALIZATION - Tiziana2019 Subject12")
    print("="*80)
    ...
    # 5. Render video
    print("[5/5] Rendering side-by-side MP4 video...")
    output_video = Path(args.output_dir) / f"worst_case_subject12_sidebyside.mp4"
    ...
    all_b3d = np.vstack([b3d_joints[i] for i in range(T)])
    all_smpl = np.vstack([smpl_joints_list[i] for i in range(T)])
    all_points = np.vstack([all_b3d, all_smpl])
    margin = 0.3
    ...
    def update(frame):
        ...
        # Left panel: B3D ground truth
        ax1 = fig.add_subplot(121, projection='3d')
        b3d = b3d_joints[frame]
        ax1.scatter(b3d[:, 0], b3d[:, 1], b3d[:, 2],
                   c='red', marker='o', s=80, alpha=0.8)

        # Connect B3D joints (simple chain)
        for i in range(len(b3d) - 1):
            ax1.plot([b3d[i, 0], b3d[i+1, 0]],
                    [b3d[i, 1], b3d[i+1, 1]],
                    [b3d[i, 2], b3d[i+1, 2]],
        ...
        ax1.set_ylabel('Y (m)')
        ax1.set_zlabel('Z (m)')
        ax1.set_title(f'B3D Ground Truth ({b3d.shape[0]} joints)', fontsize=14, color='red')
        ax1.view_init(elev=10, azim=frame * 360 / T)

        # Right panel: SMPL prediction
        ax2 = fig.add_subplot(122, projection='3d')
        smpl_j = smpl_joints_list[frame]
        ax2.scatter(smpl_j[:, 0], smpl_j[:, 1], smpl_j[:, 2],
                   c='blue', marker='^', s=100, alpha=0.8)

        # Connect SMPL joints with skeleton
        for i, j in connections:
            if i < smpl_j.shape[0] and j < smpl_j.shape[0]:
                ax2.plot([smpl_j[i, 0], smpl_j[j, 0]],
        ...
        ax2.set_ylabel('Y (m)')
        ax2.set_zlabel('Z (m)')
        ax2.set_title(f'SMPL Prediction (24 joints)', fontsize=14, color='blue')
        ax2.view_init(elev=10, azim=frame * 360 / T)

        fig.suptitle(f'Subject12 - MPJPE: {mpjpe:.2f}mm | Frame: {frame+1}/{T}',
