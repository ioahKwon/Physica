...
class OptimisationConfig:
    ...
    # REFINEMENT: Warm-start parameters
    init_from: str = None  # Path to existing result directory for warm-start
...
class AddBToSMPLFitter:
    ...
    def optimise_pose_sequence(self,
        ...
        for t in range(T):
            ...
            for iter_idx in range(num_iters):
                ...
                # WARM-START ANCHOR LOSS: Keep parameters near warm-start values
                if hasattr(self, 'warm_start_anchor_poses') and self.warm_start_anchor_poses is not None:
                    anchor_pose = self.warm_start_anchor_poses[t]  # [24, 3]
                    anchor_trans = self.warm_start_anchor_trans[t]  # [3]
                    anchor_weight = self.warm_start_anchor_weight  # Strong constraint

                    # L2 penalty to keep parameters near warm-start
                    pose_anchor_loss = ((pose_param - anchor_pose) ** 2).mean()
                    trans_anchor_loss = ((trans_param - anchor_trans) ** 2).mean()
                    loss = loss + anchor_weight * (pose_anchor_loss + trans_anchor_loss)
    ...
    def run(self) -> Tuple[torch.Tensor, np.ndarray, np.ndarray, Dict[str, float], np.ndarray]:
        ...
        REFINEMENT MODE: If warm-start data is available, skip Stage 1-2 and start from Stage 3.
        """
        print("\n" + "="*80)
        print("STARTING FOUR-STAGE ENHANCED OPTIMIZATION")
        print("="*80)

        # REFINEMENT: Load warm-start data if available
        warm_start_betas = None
        warm_start_poses = None
        warm_start_trans = None

        if self.config.init_from is not None:
            init_dir = Path(self.config.init_from)
            # Find the inner with_arm_* directory
            inner_dirs = list(init_dir.glob("with_arm_*"))
            if inner_dirs:
                data_dir = inner_dirs[0]
                npz_file = data_dir / "smpl_params.npz"

                if npz_file.exists():
                    print("\n[WARM-START] Loading existing results from smpl_params.npz...")
                    try:
                        # Load from NPZ file
                        data = np.load(npz_file)
                        warm_start_betas_np = data['betas']  # (10,)
                        warm_start_poses_np = data['poses']  # (T, 24, 3)
                        warm_start_trans_np = data['trans']  # (T, 3)

                        print(f"  Loaded shapes - betas: {warm_start_betas_np.shape}, poses: {warm_start_poses_np.shape}, trans: {warm_start_trans_np.shape}")

                        # Validate shapes
                        if warm_start_betas_np.shape[0] != 10:
                            raise ValueError(f"Invalid betas shape: {warm_start_betas_np.shape}, expected (10,)")
                        if len(warm_start_poses_np.shape) != 3 or warm_start_poses_np.shape[1:] != (24, 3):
                            raise ValueError(f"Invalid poses shape: {warm_start_poses_np.shape}, expected (T, 24, 3)")
                        if len(warm_start_trans_np.shape) != 2 or warm_start_trans_np.shape[1] != 3:
                            raise ValueError(f"Invalid trans shape: {warm_start_trans_np.shape}, expected (T, 3)")

                        # Check for NaN/Inf
                        if np.any(~np.isfinite(warm_start_betas_np)):
                            raise ValueError("betas contains NaN or Inf")
                        if np.any(~np.isfinite(warm_start_poses_np)):
                            raise ValueError("poses contains NaN or Inf")
                        if np.any(~np.isfinite(warm_start_trans_np)):
                            raise ValueError("trans contains NaN or Inf")

                        # Convert betas
                        warm_start_betas = torch.from_numpy(warm_start_betas_np).float().to(self.device)

                        # Reshape poses from (T, 24, 3) to (T, 72)
                        T_old = warm_start_poses_np.shape[0]
                        warm_start_poses_np_flat = warm_start_poses_np.reshape(T_old, -1)  # (T, 72)
                        print(f"  Reshaped poses: {warm_start_poses_np.shape} → {warm_start_poses_np_flat.shape}")

                        # Adapt to current number of frames (may be sampled differently)
                        if T_old == self.num_frames:
                            # Same frame count - use directly
                            warm_start_poses = torch.from_numpy(warm_start_poses_np_flat).float().to(self.device)
                            warm_start_trans = torch.from_numpy(warm_start_trans_np).float().to(self.device)
                            print(f"  ✓ Using data directly ({self.num_frames} frames)")
                        else:
                            # Different frame count - interpolate
                            print(f"  Adapting from {T_old} frames to {self.num_frames} frames via interpolation...")
                            from scipy.interpolate import interp1d
                            old_t = np.linspace(0, 1, T_old)
                            new_t = np.linspace(0, 1, self.num_frames)

                            # Interpolate poses (each of 72 dims)
                            poses_interp = np.zeros((self.num_frames, 72))
                            for i in range(72):
                                f = interp1d(old_t, warm_start_poses_np_flat[:, i], kind='linear', fill_value='extrapolate')
                                poses_interp[:, i] = f(new_t)

                            # Interpolate trans (each of 3 dims)
                            trans_interp = np.zeros((self.num_frames, 3))
                            for i in range(3):
                                f = interp1d(old_t, warm_start_trans_np[:, i], kind='linear', fill_value='extrapolate')
                                trans_interp[:, i] = f(new_t)

                            warm_start_poses = torch.from_numpy(poses_interp).float().to(self.device)
                            warm_start_trans = torch.from_numpy(trans_interp).float().to(self.device)
                            print(f"  ✓ Interpolation complete: {T_old} → {self.num_frames} frames")

                        print(f"  ✓ Loaded betas: {warm_start_betas.cpu().numpy()[:5]}")
                        print(f"  ✓ Loaded poses: {warm_start_poses.shape}")
                        print(f"  ✓ Loaded trans: {warm_start_trans.shape}")

                        # Quick evaluation
                        metrics_init, _ = self.evaluate(
                            warm_start_betas,
                            warm_start_poses.cpu().numpy(),
                            warm_start_trans.cpu().numpy()
                        )
                        print(f"  ✓✓✓ WARM-START SUCCESSFUL - Initial MPJPE: {metrics_init['MPJPE']:.2f} mm ✓✓✓")

                        # Set anchor for warm-start constraint (STRONG: weight=10.0)
                        # This keeps parameters VERY close to warm-start values
                        print(f"  🔒 Setting STRONG anchor constraint (weight=10.0) to keep near warm-start values")
                        self.warm_start_anchor_poses = warm_start_poses.reshape(self.num_frames, 24, 3).detach()  # [T, 24, 3]
                        self.warm_start_anchor_trans = warm_start_trans.detach()  # [T, 3]
                        self.warm_start_anchor_weight = 10.0  # STRONG constraint

                    except Exception as e:
                        print(f"\n[WARM-START] ✗ ERROR loading from {npz_file}: {e}")
                        print(f"  Falling back to random initialization")
                        warm_start_betas = None
                        warm_start_poses = None
                        warm_start_trans = None
                else:
                    print(f"\n[WARM-START] Warning: smpl_params.npz not found in {data_dir}, starting from scratch")
            else:
                print(f"\n[WARM-START] Warning: No with_arm_* directory in {init_dir}, starting from scratch")

        # Stage 1: Initial pose estimation (skip if warm-start available)
        if warm_start_poses is not None and warm_start_trans is not None:
            print("\n--- STAGE 1-2: SKIPPED (using warm-start) ---")
            initial_poses = warm_start_poses
            initial_trans = warm_start_trans
...
def parse_args() -> argparse.Namespace:
    ...
    # REFINEMENT: Warm-start from existing results
    parser.add_argument('--init_from', default=None,
                        help='Path to existing result directory for warm-start initialization')
    parser.add_argument('--refine_lr_scale', type=float, default=0.3,
                        help='Learning rate scale for refinement (default: 0.3 = 30%% of original)')
    parser.add_argument('--refine_iters_scale', type=float, default=3.0,
                        help='Iteration scale for refinement (default: 3.0 = 3× iterations)')
...
def main() -> None:
    ...
    # REFINEMENT: Apply hyperparameter scaling if warm-start is enabled
    if args.init_from:
        print(f"[REFINEMENT MODE] Warm-starting from: {args.init_from}")
        print(f"  [DEBUG] Received args.refine_lr_scale = {args.refine_lr_scale}")
        print(f"  [DEBUG] Received args.refine_iters_scale = {args.refine_iters_scale}")
        print(f"  LR scale: {args.refine_lr_scale}× (shape: {args.shape_lr * args.refine_lr_scale}, pose: {args.pose_lr * args.refine_lr_scale})")
        print(f"  Iter scale: {args.refine_iters_scale}× (shape: {int(args.shape_iters * args.refine_iters_scale)}, pose: {int(args.pose_iters * args.refine_iters_scale)})")
        args.shape_lr *= args.refine_lr_scale
        args.pose_lr *= args.refine_lr_scale
        args.shape_iters = int(args.shape_iters * args.refine_iters_scale)
        args.pose_iters = int(args.pose_iters * args.refine_iters_scale)
    ...
    if args.config:
        ...
        cfg = OptimisationConfig(
            ...
            init_from=args.init_from  # REFINEMENT: Pass warm-start path
        )
