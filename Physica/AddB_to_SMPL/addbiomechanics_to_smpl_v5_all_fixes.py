...
# -*- coding: utf-8 -*-
"""
AddBiomechanics (.b3d) → SMPL parameter fitting - v3_enhanced + Foot Orientation Loss.

This version (v3_enhanced_foot_orient_loss) adds TWO CRITICAL FIXES to v3_enhanced:
...
v3_enhanced_foot_orient_loss NEW CRITICAL FIXES:
...
FIX 2: **DIRECT FOOT ORIENTATION SUPERVISION** (NEW!)
    - Problem: Foot orientation 49° off from ground truth
    - Analysis revealed this is REAL problem, not visualization
    - Solution: Add direct foot orientation loss
        foot_dir_addb = normalize(calcn - talus)  # Ground truth
        foot_dir_smpl = normalize(foot - ankle)   # SMPL prediction
        loss = 1 - cosine_similarity(dir_addb, dir_smpl)
    - Applied to all optimization stages

Expected improvements:
    - Foot orientation: 49° → 10-15° (target)
    - MPJPE: ~75mm maintained (78mm acceptable)
...
Given an AddBiomechanics sequence we:
    ...
    3. hierarchically optimise pose + translation with all enhancements + FOOT ORIENTATION,
...
AUTO_JOINT_NAME_MAP: Dict[str, str] = {
    ...
    # FIX 4: Remove foot (mtp) from joint mapping
    # Foot position will be computed analytically from ankle + GT foot direction
    # This avoids optimization conflicts between MPJPE and foot orientation
    # 'mtp_r': REMOVED - analytical foot position
    # 'mtp_l': REMOVED - analytical foot position
    # 'subtalar_r': SKIPPED - no SMPL equivalent
    # 'subtalar_l': SKIPPED - no SMPL equivalent
    'back': 'spine2',  # FIX 5 (v5 ALL): Map to spine2 instead of spine1
    'torso': 'spine1',
    'spine': 'spine1',
...
def convert_addb_to_smpl_coords(joints: np.ndarray) -> np.ndarray:
    """
    FIX 5 (v5 Option 3): Apply 180° rotation around Y-axis

    AddB uses -X as forward, SMPL uses +X as forward.
    This rotation flips the coordinate system to align the forward directions.

    Rotation matrix around Y-axis by 180°:
    [[-1,  0,  0],
     [ 0,  1,  0],
     [ 0,  0, -1]]

    This effectively flips X and Z coordinates.
    """
    converted = joints.copy()
    # Flip X and Z axes
    converted[..., 0] = -joints[..., 0]  # X → -X
    converted[..., 2] = -joints[..., 2]  # Z → -Z
    # Y remains unchanged (up axis)
...
class OptimisationConfig:
    ...
    # FIX 5 (v5 ALL FIXES): Combined approach
    # - 180° coordinate rotation (in convert_addb_to_smpl_coords)
    # - spine2 mapping (in ADDB_TO_SMPL_MAP)
    # - Pelvis upright constraint
    # - Spine vertical constraint
    # - Ankle rotation limits
    pelvis_upright_weight: float = 10.0  # Strong constraint to keep pelvis upright
    spine_vertical_weight: float = 10.0  # Enforce vertical spine alignment
...
class AddBToSMPLFitter:
    ...
    def _compute_pelvis_upright_loss(self, poses: torch.Tensor) -> torch.Tensor:
        ...
        Penalize pelvis rotation away from upright posture.
        Limits pitch and roll to keep upper body vertical.

        Args:
            poses: Pose parameters [T, 24, 3] (axis-angle)

        Returns:
            Pelvis upright loss (scalar)
        """
        pelvis_pose = poses[:, 0, :]  # [T, 3] - pelvis rotation

        # Penalize X and Z rotations (pitch and roll)
        # These cause forward/backward and side-to-side lean
        pitch_roll_penalty = (pelvis_pose[:, 0] ** 2 + pelvis_pose[:, 2] ** 2).mean()

        # Optional: Allow some Y rotation (twist around vertical axis)
        # Y rotation is natural during walking, so we don't penalize it as much

        return pitch_roll_penalty

    def _compute_spine_vertical_loss(self, joints: torch.Tensor) -> torch.Tensor:
        """
        FIX 5 (v5 ALL): Spine Vertical Constraint
        Enforce that the spine vector points upward (vertical).
        Prevents forward/backward lean of the upper body.

        Args:
            joints: Joint positions [T, 24, 3] or [24, 3]

        Returns:
            Spine vertical loss (scalar)
        """
        ...
        # Compute spine direction vectors
        spine_vec1 = spine1 - pelvis
        spine_vec2 = spine2 - spine1
        spine_vec3 = spine3 - spine2

        # Vertical direction is [0, 1, 0] (Y-axis up)
        # Penalize deviation from vertical
        if is_sequence:
            # For each vector, penalize X and Z components (should be near 0)
            # Y component should be positive and large
            loss = (
                (spine_vec1[:, 0] ** 2 + spine_vec1[:, 2] ** 2).mean() +
                (spine_vec2[:, 0] ** 2 + spine_vec2[:, 2] ** 2).mean() +
                (spine_vec3[:, 0] ** 2 + spine_vec3[:, 2] ** 2).mean()
            ) / 3.0
        else:
            loss = (
                (spine_vec1[0] ** 2 + spine_vec1[2] ** 2) +
                (spine_vec2[0] ** 2 + spine_vec2[2] ** 2) +
                (spine_vec3[0] ** 2 + spine_vec3[2] ** 2)
            ) / 3.0
    ...
    def optimise_with_sequence_enhancements(self,
        ...
        for iter_idx in range(num_iters):
            ...
            # FIX 5 (v5 ALL): Pelvis Upright Constraint
            ...
            # FIX 5 (v5 ALL): Spine Vertical Constraint
            spine_vertical_loss = self._compute_spine_vertical_loss(pred_joints_tensor)
            total_loss = total_loss + self.config.spine_vertical_weight * spine_vertical_loss
            ...
            if (iter_idx + 1) % 10 == 0:
                print(f'    Iter {iter_idx+1}/{num_iters}: Loss={total_loss.item():.6f}, '
                      ...
                      f'Pelvis={pelvis_upright_loss.item():.6f}, Spine={spine_vertical_loss.item():.6f}, Ankle={ankle_limit_loss.item():.6f}')
...
def main() -> None:
    ...
    # Add prefix to distinguish from baseline v3_enhanced
    run_name = "foot_orient_loss_" + run_name
