#!/usr/bin/env python3
"""
3D mesh visualization for SMPL fitting results using PyTorch SMPL
Properly renders SMPL body mesh with shape AND pose using kinematic tree
"""
import numpy as np
...
def get_mesh_edges(vertices, faces, sample_rate=20):
    """
...
def create_mesh_animation(result_dir, smpl_model_path, output_path=None,
                          ...
                          device='cpu'):
    """
    Create 3D mesh animation with SMPL body shape using PyTorch SMPL
    ...
    Args:
        ...
        wireframe_density: Sample every Nth face for wireframe
        device: 'cpu' or 'cuda'
    """
    # Load data
    ...
    # Load SMPL model with PyTorch
    print(f"Loading SMPL model from {smpl_model_path}...")
    device_torch = torch.device(device)
    smpl_model = SMPLModel(smpl_model_path, device=device_torch)
    ...
    # Compute SMPL mesh for all frames using PyTorch forward pass
    print(f"Computing SMPL mesh for {num_frames} frames with PyTorch...")
    vertices_all = []

    # Convert to torch tensors
    betas_torch = torch.from_numpy(betas).float().to(device_torch).unsqueeze(0)  # (1, 10)

    with torch.no_grad():
        for t in range(num_frames):
            # Reshape pose from (24, 3) to (72,) if needed
            pose = poses[t]
            if pose.ndim == 2:  # (24, 3) -> flatten to (72,)
                pose = pose.reshape(-1)

            pose_torch = torch.from_numpy(pose).float().to(device_torch).unsqueeze(0)  # (1, 72)
            trans_torch = torch.from_numpy(trans[t]).float().to(device_torch).unsqueeze(0)  # (1, 3)

            # SMPL forward pass with FULL pose applied
            vertices, _ = smpl_model.forward(betas_torch, pose_torch, trans_torch)
            vertices_all.append(vertices[0].cpu().numpy())  # (6890, 3)

    vertices_all = np.array(vertices_all)  # (T, 6890, 3)
    faces = smpl_model.faces.cpu().numpy()

    print(f"Mesh computed: {vertices_all.shape[0]} frames, {vertices_all.shape[1]} vertices")
    ...
    def update(frame_data):
        ...
        title_suffix = "FULL BODY MESH (with Pose)" if body_type == "with_arm" else "LOWER BODY MESH (with Pose)"
    ...
    if output_path is None:
        ...
        output_path = Path(result_dir) / f'{subject_name}_3d_mesh_posed.mp4'
    else:
...
def main():
    parser = argparse.ArgumentParser(
        description='Create 3D mesh visualization with SMPL body shape AND pose (PyTorch)'
    )
