...
def interpolate_poses_cubic(
    ...
    """
    Cubic spline interpolation for axis-angle poses.

    Args:
        poses: (T, 24, 3) axis-angle poses (only keyframe_indices are valid)
        keyframe_indices: List of frame indices that were optimized
        total_frames: Total number of frames (T)
        device: torch device

    Returns:
        Interpolated poses (T, 24, 3) with all frames filled

    Note: For simplicity and speed, we perform axis-angle direct interpolation.
    For higher accuracy, consider quaternion SLERP or rotation matrix interpolation.
    """
    from scipy.interpolate import CubicSpline

    result = torch.zeros(total_frames, 24, 3, device=device)
    result[keyframe_indices] = poses[keyframe_indices]

    # Convert keyframe indices to numpy for scipy
    keyframe_indices_np = np.array(keyframe_indices)

    # Interpolate each joint's 3 axes independently
    for joint_idx in range(24):
        for axis_idx in range(3):
            # Extract keyframe values
            keyframe_values = poses[keyframe_indices, joint_idx, axis_idx].cpu().numpy()

            # Create cubic spline
            cs = CubicSpline(keyframe_indices_np, keyframe_values)

            # Interpolate all frames
            all_frames = np.arange(total_frames)
            interpolated = cs(all_frames)

            # Write back to result
            result[:, joint_idx, axis_idx] = torch.from_numpy(interpolated).to(device)
...
class OptimisationConfig:
    ...
    pose_subsample_rate: int = 1  # NEW: Subsample every Nth frame in Stage 3 (1=no subsampling)

    # Stage 4 (Sequence Enhancement) optimization parameters
    sequence_enhancement_iters: int = 50  # Number of iterations for Stage 4
    sequence_subsample_rate: int = 1  # Subsample frames in Stage 4 (1=no subsampling)
    sequence_batch_size: int = -1  # Mini-batch size for Stage 4 (-1=use all frames)
    ...
    # Physics Loss (NEW): Enable physics-based loss terms
    use_physics_loss: bool = False  # Enable/disable physics loss
    physics_foot_height_weight: float = 0.01  # Foot height consistency (when GRF > 0) - REDUCED
    physics_com_weight: float = 0.005  # CoM trajectory matching - REDUCED
    physics_vel_smooth_weight: float = 0.001  # Velocity smoothness - REDUCED
    physics_loss_stages: List[int] = field(default_factory=lambda: [3, 4])  # Apply physics only in stages 3-4
...
class AddBToSMPLFitter:
    def __init__(self,
                 ...
                 b3d_path: Optional[str] = None) -> None:
        ...
        # Initialize SMPL body segment weights for CoM calculation
        self._init_smpl_segment_weights()

        # Track current optimization stage for physics loss (1-4)
        self.current_stage = 1

        # Load physics data if physics loss is enabled
        self.physics_data = None
        if self.config.use_physics_loss and b3d_path is not None:
            self._load_physics_data(b3d_path)

    # --------------------------- Physics Loss (NEW) -----------------------
    def _init_smpl_segment_weights(self) -> None:
        """
        Initialize body segment mass distribution for proper CoM calculation.
        Based on anthropometric data (Winter's body segment parameters).
        SMPL has 24 joints (0-23), we assign approximate mass weights.
        """
        # Approximate mass distribution for SMPL joints (normalized to sum to 1.0)
        # Based on Winter's body segment mass percentages of total body mass
        self.joint_weights = torch.zeros(24, device=self.device)

        # Pelvis (root) - trunk mass centered here: ~50% of body mass
        self.joint_weights[0] = 0.50

        # Legs (each leg ~16% of body mass, distributed across joints)
        # Left leg
        self.joint_weights[1] = 0.10  # left_hip
        self.joint_weights[4] = 0.04  # left_knee
        self.joint_weights[7] = 0.01  # left_ankle
        self.joint_weights[10] = 0.01  # left_foot

        # Right leg
        self.joint_weights[2] = 0.10  # right_hip
        self.joint_weights[5] = 0.04  # right_knee
        self.joint_weights[8] = 0.01  # right_ankle
        self.joint_weights[11] = 0.01  # right_foot

        # Spine/neck (distributed ~5% additional mass)
        self.joint_weights[3] = 0.02  # spine1
        self.joint_weights[6] = 0.01  # spine2
        self.joint_weights[9] = 0.01  # spine3
        self.joint_weights[12] = 0.005  # neck
        self.joint_weights[15] = 0.005  # head

        # Arms (each arm ~5% of body mass)
        # Left arm
        self.joint_weights[13] = 0.02  # left_collar
        self.joint_weights[16] = 0.015  # left_shoulder
        self.joint_weights[18] = 0.01  # left_elbow
        self.joint_weights[20] = 0.005  # left_wrist

        # Right arm
        self.joint_weights[14] = 0.02  # right_collar
        self.joint_weights[17] = 0.015  # right_shoulder
        self.joint_weights[19] = 0.01  # right_elbow
        self.joint_weights[21] = 0.005  # right_wrist

        # Hands
        self.joint_weights[22] = 0.003  # left_hand
        self.joint_weights[23] = 0.003  # right_hand

        # Normalize to sum to 1.0
        self.joint_weights = self.joint_weights / self.joint_weights.sum()

    def _compute_smpl_com(self, joints: torch.Tensor) -> torch.Tensor:
        """
        Compute center of mass from SMPL joints using body segment weights.
        ...
        com = (joints * self.joint_weights.unsqueeze(1)).sum(dim=0)
        return com

    def _load_physics_data(self, b3d_path: str) -> None:
        """Load physics data from .b3d file using extract_simple_physics_from_b3d"""
        try:
            from extract_addbiomechanics_physics import extract_simple_physics_from_b3d

            print(f"  [Physics] Loading physics data from {b3d_path}...")

            # Extract physics data (DYNAMICS pass = most accurate)
            self.physics_data = extract_simple_physics_from_b3d(
                b3d_path=b3d_path,
                trial=0,
                processing_pass=2,  # DYNAMICS
                start_frame=0,
                num_frames=self.target_joints_np.shape[0]
            )

            # Convert to torch tensors
            import torch
            self.physics_data['grf'] = torch.from_numpy(self.physics_data['grf']).float().to(self.device)
            self.physics_data['com_pos'] = torch.from_numpy(self.physics_data['com_pos']).float().to(self.device)
            self.physics_data['com_vel'] = torch.from_numpy(self.physics_data['com_vel']).float().to(self.device)
            self.physics_data['com_acc'] = torch.from_numpy(self.physics_data['com_acc']).float().to(self.device)

            grf_magnitude = torch.norm(self.physics_data['grf'], dim=1).mean().item()
            print(f"  [Physics] Loaded {self.physics_data['num_frames']} frames")
            print(f"  [Physics] GRF mean magnitude: {grf_magnitude:.2f} N")
            print(f"  [Physics] Weights: foot_height={self.config.physics_foot_height_weight}, "
                  f"com={self.config.physics_com_weight}, vel_smooth={self.config.physics_vel_smooth_weight}")

        except Exception as e:
            print(f"  [Physics] WARNING: Failed to load physics data: {e}")
            print(f"  [Physics] Continuing without physics loss...")
            self.physics_data = None

    def compute_physics_loss(self, smpl_joints: torch.Tensor, frame_idx: int, stage: int = 1) -> torch.Tensor:
        """
        Compute physics-based loss with IMPROVED implementation:
        - Proper CoM calculation using body segment weights
        - Soft thresholds for foot height and GRF
        - Applied only in later stages (3-4) for better convergence

        Args:
            smpl_joints: (24, 3) SMPL joint positions
            frame_idx: Current frame index
            stage: Current optimization stage (1-4)

        Returns:
            total_loss: Weighted sum of physics losses
        """
        # Only apply physics loss in specified stages (default: 3-4)
        if stage not in self.config.physics_loss_stages:
            return torch.tensor(0.0, device=self.device)

        if self.physics_data is None or frame_idx >= self.physics_data['num_frames']:
            return torch.tensor(0.0, device=self.device)

        total_loss = torch.tensor(0.0, device=self.device)

        # 1. Foot Height Consistency with SOFT threshold
        # Use sigmoid to create smooth transition instead of hard threshold
        grf_magnitude = torch.norm(self.physics_data['grf'][frame_idx])

        # Soft GRF threshold: sigmoid centered at 50N with width 20N
        grf_weight = torch.sigmoid((grf_magnitude - 50.0) / 20.0)

        left_foot_height = smpl_joints[10, 2]  # Z-axis
        right_foot_height = smpl_joints[11, 2]

        # Soft height penalty: smooth transition around 0.05m with width 0.02m
        left_penalty = torch.sigmoid((left_foot_height - 0.05) / 0.02) * left_foot_height
        right_penalty = torch.sigmoid((right_foot_height - 0.05) / 0.02) * right_foot_height

        foot_height_loss = grf_weight * (left_penalty + right_penalty)
        total_loss += self.config.physics_foot_height_weight * foot_height_loss

        # 2. CoM Trajectory Matching using PROPER body segment weights
        pred_com = self._compute_smpl_com(smpl_joints)
        target_com = self.physics_data['com_pos'][frame_idx]
        com_loss = F.mse_loss(pred_com, target_com)
        total_loss += self.config.physics_com_weight * com_loss

        # 3. Velocity Smoothness (temporal smoothness)
        # TODO: Implement if needed - requires storing previous frame joints
    ...
    def optimise_pose_sequence(self,
        ...
        subsample_rate = self.config.pose_subsample_rate

        # Generate subsample indices
        if subsample_rate > 1:
            subsample_indices = list(range(0, T, subsample_rate))
            # Always include last frame
            if subsample_indices[-1] != T - 1:
                subsample_indices.append(T - 1)
            print(f'  [Stage 3 Subsampling] Optimizing {len(subsample_indices)}/{T} frames (rate={subsample_rate})')
        else:
            subsample_indices = list(range(T))

        for t in subsample_indices:
            target_frame = self.target_joints[t]
            if torch.isnan(target_frame).all():
                continue
            ...
            for iter_idx in range(num_iters):
                ...
                # Physics loss (NEW) - using current_stage
                if self.config.use_physics_loss and self.physics_data is not None:
                    physics_loss = self.compute_physics_loss(joints_pred, t, stage=self.current_stage)
        ...
        # Interpolate skipped frames if subsampling was used
        if subsample_rate > 1 and len(subsample_indices) < T:
            print(f'  [Stage 3 Interpolation] Interpolating {T - len(subsample_indices)} skipped frames...')

            # Interpolate poses
            poses = interpolate_poses_cubic(poses, subsample_indices, T, self.device)

            # Interpolate translations (simpler linear interpolation is fine for translation)
            from scipy.interpolate import CubicSpline
            subsample_indices_np = np.array(subsample_indices)
            trans_np = trans.cpu().numpy()

            for axis_idx in range(3):
                keyframe_values = trans_np[subsample_indices, axis_idx]
                cs = CubicSpline(subsample_indices_np, keyframe_values)
                all_frames = np.arange(T)
                trans_np[:, axis_idx] = cs(all_frames)

            trans = torch.from_numpy(trans_np).to(self.device)

            print(f'  [Stage 3 Interpolation] Complete')
    ...
    def optimise_with_sequence_enhancements(self,
                                             ...
                                             num_iters: int = None) -> Tuple[np.ndarray, np.ndarray]:
        """
        ...
        Args:
            ...
            num_iters: Number of refinement iterations (None=use config)

        Returns:
            (refined_poses, refined_trans) as numpy arrays
        """
        if num_iters is None:
            num_iters = self.config.sequence_enhancement_iters

        subsample_rate = self.config.sequence_subsample_rate
        batch_size = self.config.sequence_batch_size

        print(f'  [Stage 4 Sequence Enhancement] iters={num_iters}, subsample={subsample_rate}, batch={batch_size}')

        betas = betas.detach()
        T = self.target_joints.shape[0]

        # Generate subsample indices for Stage 4
        if subsample_rate > 1:
            subsample_indices = list(range(0, T, subsample_rate))
            if subsample_indices[-1] != T - 1:
                subsample_indices.append(T - 1)
            print(f'  [Stage 4 Subsampling] Optimizing {len(subsample_indices)}/{T} frames')
        else:
            subsample_indices = list(range(T))
        ...
        # Only detect on subsampled frames for efficiency
        with torch.no_grad():
            pred_joints_all = []
            for t in subsample_indices:
                jp = self.smpl.joints(betas.unsqueeze(0), poses[t].unsqueeze(0), trans[t].unsqueeze(0)).squeeze(0)
                pred_joints_all.append(jp)
            pred_joints_sampled = torch.stack(pred_joints_all, dim=0)  # [len(subsample_indices), 24, 3]
            contact_masks_sampled = self._detect_foot_contact(pred_joints_sampled)  # [len(subsample_indices), 2]

            # Create full contact mask array (initialize all to False)
            contact_masks = torch.zeros(T, 2, dtype=torch.bool, device=self.device)
            for idx, t in enumerate(subsample_indices):
                contact_masks[t] = contact_masks_sampled[idx]

        for iter_idx in range(num_iters):
            optimizer.zero_grad()

            # Mini-batch sampling: select random subset of frames if batch_size is set
            if batch_size > 0 and batch_size < len(subsample_indices):
                import random
                batch_indices = random.sample(subsample_indices, batch_size)
            else:
                batch_indices = subsample_indices

            # Compute joints for subsampled/batched frames only
            pred_joints_all = []
            for t in batch_indices:
                jp = self.smpl.joints(betas.unsqueeze(0), poses[t].unsqueeze(0), trans[t].unsqueeze(0)).squeeze(0)
                pred_joints_all.append(jp)
            pred_joints_tensor = torch.stack(pred_joints_all, dim=0)  # [batch_size, 24, 3]
            ...
            for batch_idx, t in enumerate(batch_indices):
                ...
                joints_pred = pred_joints_tensor[batch_idx]  # Use batch_idx, not t!
                ...
                # Use contact mask from initial detection (contact_masks indexed by t)
                if t < len(contact_masks):
        ...
        # Interpolate skipped frames if subsampling was used
        if subsample_rate > 1 and len(subsample_indices) < T:
            print(f'  [Stage 4 Interpolation] Interpolating {T - len(subsample_indices)} skipped frames...')

            # Interpolate poses
            poses_np = poses.detach().cpu().numpy()
            poses_interpolated = interpolate_poses_cubic(
                torch.from_numpy(poses_np).to(self.device),
                subsample_indices,
                T,
                self.device
            )
            ...
            trans_interpolated = np.zeros((T, 3), dtype=np.float32)
            for axis_idx in range(3):
                keyframe_values = trans_np[subsample_indices, axis_idx]
                cs = CubicSpline(subsample_indices_np, keyframe_values)
                all_frames = np.arange(T)
                trans_interpolated[:, axis_idx] = cs(all_frames)

            print(f'  [Stage 4 Interpolation] Complete')
            return poses_interpolated.cpu().numpy(), trans_interpolated
        else:
    ...
    def run(self) -> Tuple[torch.Tensor, np.ndarray, np.ndarray, Dict[str, float], np.ndarray]:
        ...
        poses_np, trans_np = self.optimise_with_sequence_enhancements(
            ...
            # num_iters will be taken from config
        )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument('--pose_subsample_rate', type=int, default=1,
                        help='Subsample every Nth frame in Stage 3 pose optimization (1=no subsampling, 48=recommended for speed)')
    ...
    # Stage 4 (Sequence Enhancement) parameters
    parser.add_argument('--sequence_enhancement_iters', type=int, default=50,
                        help='Number of iterations for Stage 4 sequence enhancement (default: 50)')
    parser.add_argument('--sequence_subsample_rate', type=int, default=1,
                        help='Subsample rate for Stage 4 (1=no subsampling, 4=recommended for speed)')
    parser.add_argument('--sequence_batch_size', type=int, default=-1,
                        help='Mini-batch size for Stage 4 (-1=use all frames, 200=recommended for speed)')

    # Physics loss parameters (NEW)
    parser.add_argument('--use_physics_loss', action='store_true',
                        help='Enable physics-based loss')
    parser.add_argument('--physics_foot_height_weight', type=float, default=0.1,
                        help='Foot height consistency weight (default: 0.1)')
    ...
    parser.add_argument('--physics_vel_smooth_weight', type=float, default=0.02,
                        help='Velocity smoothness weight (default: 0.02)')
...
def process_single_b3d(
    ...
    fitter = AddBToSMPLFitter(
        ...
        config=config,
        device=device,
        b3d_path=b3d_path  # Pass b3d_path for physics data loading
    )
...
def main() -> None:
    ...
    cfg = OptimisationConfig(
        ...
        pose_subsample_rate=args.pose_subsample_rate,
        tolerance_mm=args.tolerance_mm,
        max_passes=args.max_passes,
        pose_reg_weight=args.pose_reg,
        trans_reg_weight=args.trans_reg,
        sequence_enhancement_iters=args.sequence_enhancement_iters,
        sequence_subsample_rate=args.sequence_subsample_rate,
