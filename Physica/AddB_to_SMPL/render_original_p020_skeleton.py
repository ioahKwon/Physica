#!/usr/bin/env python3
"""
Render SMPL skeleton from ORIGINAL P020 analytical foot results
(The one that was used for smpl_joints_p020_analytical_foot.mp4)
"""
...
def project_3d_to_2d(points_3d, img_width, img_height, scale=500, center_point=None):
    ...
    Args:
        ...
        center_point: [3] point to center the camera on (e.g., pelvis position)
    """
    # Orthographic projection (ignore Z for now, just use X and Y)
    points_2d = points_3d[:, [0, 1]].copy()

    # If center_point is provided, offset to follow the subject
    if center_point is not None:
        center_2d = center_point[[0, 1]]
        points_2d -= center_2d  # Move to origin
...
def render_frame(joints_3d, img_width, img_height):
    ...
    # Get pelvis position (joint 0) for camera following
    pelvis = joints_3d[0]

    # Project joints to 2D with camera following pelvis
...
def main():
    # Use FIX 7 split5 result
    smpl_params_path = '/tmp/v7_p020_split5/foot_orient_loss_with_arm_carter2023_p020/smpl_params.npz'
    smpl_model_path = '/egr/research-zijunlab/kwonjoon/Code/Physica/AddB_to_SMPL/models/smpl_model.pkl'
    output_path = '/egr/research-zijunlab/kwonjoon/smpl_skeleton_v7_p020_split5.mp4'

    print("=" * 80)
    print("FIX 7 (No Spine Mapping) - P020_split5 Skeleton Video")
    print("=" * 80)
    ...
    for i in range(len(joints)):
        ...
        cv2.putText(img, 'FIX 7: P020_split5', (10, 70),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 150, 0), 2)
        cv2.putText(img, f'MPJPE: 19.10mm', (10, 110),
