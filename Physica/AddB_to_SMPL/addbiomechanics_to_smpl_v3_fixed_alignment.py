...
# -*- coding: utf-8 -*-
"""
AddBiomechanics (.b3d) → SMPL parameter fitting - Fixed Global Alignment Version.

This version (v3_fixed_alignment) fixes the critical global skeleton misalignment problem
where SMPL ends up in wrong position (left side) while AddB ground truth is on right side.
...
v3_enhanced features (baseline):
    ...
    → Achieved: 74.62mm MPJPE but WRONG global position

v3_fixed_alignment NEW fixes:
    FIX 1: Strong translation regularization in Stage 1 (weight=1.0)
           - Prevents translation from drifting during initial optimization
           - Anchors translation to AddB pelvis position

    FIX 2: Increased global trans_reg_weight from 0.001 to 1.0
           - Makes translation regularization 1000x stronger
           - Comparable to position loss magnitude (~10-100)
           - Maintains correct global position throughout all stages

Problem addressed:
    - Professor feedback: "foot is toward the back" and "foot is not optimized"
    - Root cause: GLOBAL MISALIGNMENT - entire skeleton in wrong position
    - Translation regularization was too weak (0.001) or absent in Stage 1
    - Translation drifted from correct initial position during optimization

Expected improvements:
    - Correct global skeleton alignment (SMPL and AddB in same position)
    - Feet will appear correctly oriented (no more "pointing backward")
...
Given an AddBiomechanics sequence we:
    ...
    3. optimise pose + translation with STRONG translation anchoring,
    4. apply sequence-level refinement maintaining global alignment.
...
class OptimisationConfig:
    ...
    trans_reg_weight: float = 1.0  # FIX 2: Increased from 0.001 to 1.0 (1000x stronger)
    ...
    def optimise_initial_poses(self) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        for t in range(T):
            ...
            for _ in range(num_iters):
                ...
                # FIX 1: Strong translation regularization (anchor to pelvis) - CRITICAL FIX
                # This prevents translation from drifting during Stage 1 optimization
                target_pelvis_idx = 0  # ground_pelvis in AddBiomechanics
                addb_indices_list = self.addb_indices.cpu().tolist()
                if target_pelvis_idx in addb_indices_list:
                    addb_pelvis_pos = addb_indices_list.index(target_pelvis_idx)
                    target_pelvis = target_frame[addb_pelvis_pos]
                    if not torch.isnan(target_pelvis).any():
                        loss = loss + 1.0 * ((trans_param - target_pelvis) ** 2).mean()  # Weight 1.0 = strong anchor
...
def main() -> None:
    ...
    run_name = "fixed_align_" + run_name
