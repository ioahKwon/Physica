#!/usr/bin/env python3
"""
Create 3D rotating visualization of SMPL fitting results
"""
import numpy as np
...
# SMPL skeleton connections for visualization
SMPL_SKELETON = [
    (0, 1), (0, 2), (0, 3),  # pelvis to hips and spine
    (1, 4), (2, 5),  # hips to knees
    (4, 7), (5, 8),  # knees to ankles
    (7, 10), (8, 11),  # ankles to feet
    (3, 6), (6, 9),  # spine to neck to head
    (9, 12), (9, 13), (9, 14),  # head connections
    (12, 15), (13, 16), (14, 17),  # upper body
    (16, 18), (17, 19),  # arms to hands
    (18, 20), (19, 21),  # hands
    (20, 22), (21, 23),  # fingers
]
...
def create_3d_animation(result_dir, output_path=None, fps=30, rotate_speed=2.0):
    """
    Create 3D rotating animation

    Args:
        result_dir: Path to result directory
        output_path: Output MP4 path
        fps: Frames per second
        rotate_speed: Rotation speed in degrees per frame
    """
    ...
    all_joints = np.concatenate([pred_joints.reshape(-1, 3),
    ...
    def update(frame_data):
        ...
        valid_pred = pred[~np.isnan(pred).any(axis=1)]
        if len(valid_pred) > 0:
            ax.scatter(valid_pred[:, 0], valid_pred[:, 1], valid_pred[:, 2],
                      c='blue', s=50, marker='o', label='Predicted (SMPL)', alpha=0.8)

        # Plot target joints
        valid_target = target_vis[~np.isnan(target_vis).any(axis=1)]
        if len(valid_target) > 0:
            ax.scatter(valid_target[:, 0], valid_target[:, 1], valid_target[:, 2],
                      c='red', s=50, marker='^', label='Target (AddB)', alpha=0.8)
    ...
    print(f'Creating 3D rotating animation with {num_frames} frames...')
    anim = FuncAnimation(fig, update, frames=frame_data,
                        interval=1000/fps, blit=False)

    # Save
    if output_path is None:
        output_path = Path(result_dir) / 'animation_3d.mp4'
    else:
...
def main():
    parser = argparse.ArgumentParser(description='Create 3D rotating visualization')
    ...
    parser.add_argument('--rotate_speed', type=float, default=2.0,
                       help='Rotation speed in degrees per frame (default: 2.0)')

    args = parser.parse_args()
    create_3d_animation(args.result_dir, args.output, args.fps, args.rotate_speed)
